...
router.get('/users', async (_req, res) => {
  try {
    const users = await User.find().select('name email phone city userType sellerType businessName role status phoneVerified isVerifiedSeller verificationDate createdAt');
    res.json(users);
  } catch (error) {
...
router.patch('/users/:id/verify-seller', async (req, res) => {
  try {
    ...
    user.isVerifiedSeller = isVerifiedSeller === true;
    user.verificationDate = isVerifiedSeller === true ? new Date() : null;
    await user.save();
    res.json({ 
      message: `Seller ${isVerifiedSeller ? 'verified' : 'unverified'}`, 
      user: {
        _id: user._id,
        name: user.name,
        email: user.email,
